...
class _GridAttentionBlockND(nn.Module):
    ...
    def _concatenation(self, x, g):
        ...
        print(input_size,batch_size,g.size(0))
...
#         print(f.size())
        ...
        if self.mode == 'concatenation_softmax':
            ...
            psi_f_flat = psi_f.view(batch_size, 1, -1)
#             print('psi_f_flat',psi_f_flat.shape)
            ss = psi_f_flat.shape
            ...
            sigm_psi_f = psi_f_flat / psi_f_sum
            sigm_psi_f = sigm_psi_f.view(batch_size, 1, *theta_x_size[2:])
            print('sigm_psi_fV0',sigm_psi_f.shape)
        ...
        sigm_psi_f = F.upsample(sigm_psi_f, size=input_size[2:], mode=self.upsample_mode)
        print('sigm_psi_fV1',sigm_psi_f.shape)
        print('sigm_psi_fV2',sigm_psi_f.expand_as(x).shape)
        y = sigm_psi_f.expand_as(x) * x
        W_y = self.W(y)
